...
function changeDialValue (index, incrementBy) {
  ...
  let result = checkCombo(lockState.wheels,lockState.wheels.length - 1);
  console.log(result);
  if(result){
    console.log('unlocked');
    lockState.locked = false;
    redirect('chris-hjorth');
  }
...
function checkCombo(input,index, result = false){
  console.log('checking')
  if(input.length > 0){
    if(input[index] === SECRET_COMBO[index]){
      input.pop();
      checkCombo(input,index - 1);
    }
    else{
      return false;
    }
